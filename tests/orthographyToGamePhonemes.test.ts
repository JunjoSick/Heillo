...
describe("orthographyToGamePhonemes", () => {
  it("handles roadmap transcription examples", () => {
    ...
    expect(transcribe("quasi")).toEqual(["K", "W", "A", "S", "I"]);
    expect(transcribe("mai")).toEqual(["M", "A", "J"]);
    expect(transcribe("auto")).toEqual(["A", "W", "T", "O"]);
    expect(transcribe("palla")).toEqual(["P", "A", "LL", "A"]);
    expect(transcribe("fatto")).toEqual(["F", "A", "TT", "O"]);
    expect(transcribe("cassa")).toEqual(["K", "A", "SS", "A"]);
    ...
    expect(transcribe("aglio")).toEqual(["A", "GL", "O"]);
    expect(transcribe("uomo")).toEqual(["W", "O", "M", "O"]);
